...
export function serveStatic(app: Express) {
  ...
  // Check if dist path exists, if not try alternative paths
  ...
  if (!fs.existsSync(finalDistPath)) {
    ...
    // In Vercel environment, we might not need to serve static files as they're handled differently
    if (process.env.VERCEL) {
      console.log("Running on Vercel, static files are handled by Vercel directly");
      return;
    }
    return;
  }
